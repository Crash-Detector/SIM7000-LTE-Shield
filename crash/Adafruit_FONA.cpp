...
boolean Adafruit_FONA::begin(Stream &port) {
  ...
  DEBUG_PRINTLN(F("Before while loop"));
  while (timeout > 0) {
    while (mySerial->available()) 
       {
        DEBUG_PRINTLN(F("Reseial->available() module..."));
         mySerial->read();
        DEBUG_PRINTLN(F("After read..."));
       } // end while
    DEBUG_PRINTLN(F("Outside mySerial->available()"));
...
boolean Adafruit_FONA::sendSMS(const char *smsaddr, const char *smsmsg) {
  if (! sendCheckReply(F("AT+CMGF=1"), ok_reply)) 
      {
      DEBUG_PRINTLN("Failed sendCheckReply");
      return false;
      } // end if
